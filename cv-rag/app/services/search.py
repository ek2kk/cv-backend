...
def build_index(
    ...
    for path in md_dir.glob("*.md"):
        ...
        docs.append(text)
        metadata.append(SearchResult(file=path.name, score=0.0, title=title, text=text))
...
def ensure_index(
    ...
    if not is_index_ready(index_path, meta_path):
        logger.info("Search index is missing, building it")
...
@lru_cache(maxsize=1)
def _load_index(index_path: str, meta_path: str) -> tuple[Any, list[SearchResult]]:
    started_at = perf_counter()
    index = faiss.read_index(index_path)
    with Path(meta_path).open("r", encoding="utf-8") as f:
        metadata = [SearchResult.model_validate(item) for item in json.load(f)]
    logger.info(
        "Search index loaded",
        extra={
            "extra_data": {
